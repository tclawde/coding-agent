#!/usr/bin/env python3
"""Moltbook community interaction at 22:00 - Full execution"""
import http.client
import json
import ssl
import time
...
def get_my_posts(agent_id):
    """Get my posts"""
    return api_request(f"/api/v1/agents/{agent_id}/posts?limit=10")
...
def main():
    ...
    stats = {"comments_replied": 0, "liked": 0, "commented": 0, "welcomed": 0, "my_posts": []}
    ...
    my_posts_data = get_my_posts(my_id)
    if my_posts_data and my_posts_data.get('success'):
        my_posts = my_posts_data.get('data', [])
        stats['my_posts'] = my_posts
